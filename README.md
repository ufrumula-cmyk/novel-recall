...
popup 页面提供 `AI 猜书` 模式。用户可以输入记得的剧情描述，让 SiliconFlow Chat Completion 生成可能的小说候选。

需要特别注意：

- AI 猜书是开放候选生成，不是数据库检索。
- 信息不足、描述太泛或缺少关键剧情元素时，AI 猜书会优先追问，而不是强行生成候选。
- 如果描述包含多个具体元素，例如时代背景、人物关系、身份设定和关键事件，AI 猜书可以返回低置信度或中置信度候选。
- 结果可能出现幻觉或错误，需要用户自行核验。
- 候选会醒目标注 `AI 推测，未验证`。
- AI 猜书不联网搜索，不访问晋江、番茄、起点或其他小说平台。
- AI 猜书不采集网页正文、小说章节正文、免费章节正文、VIP/付费章节正文。
- AI 猜书不生成 embedding，也不读取本地小说库来匹配候选。
- AI 猜书结果暂时只在 popup 展示，不写入 IndexedDB，不导出 JSON。

它和本地剧情语义检索的区别：

- 本地语义检索基于用户已经导入 Novel Recall 的小说数据，使用 query embedding 与本地小说 embedding 做相似度排序。
- AI 猜书基于大模型对剧情描述的推测生成候选，不代表候选真实存在或已经核验。
