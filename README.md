...
它不来自真实小说平台，不来自 Hugging Face 或 Chinese-web-novel 抽样数据，不包含完整小说正文，可用于 GitHub README、录屏和公开项目展示。
...
- 从 0 到 1 完成面向小说简介与剧情描述的浏览器扩展检索系统。
- 使用 IndexedDB 实现本地优先的数据持久化，支持小说数据的导入、导出、删除与清空。
- 接入 SiliconFlow Chat Completion，对小说简介生成摘要、剧情关键词、角色标签与类型标签。
- 接入 Embeddings，并在本地使用 cosine similarity 实现剧情语义检索。
- 将 popup 检索入口与 options 管理后台拆分，兼顾轻量搜索和数据管理。
- 提供公开数据集转换脚本，并避免保存完整正文，降低版权和合规风险。
- 明确项目边界：不爬取平台正文、不模拟登录、不绕过反爬、不采集付费或受保护内容。
