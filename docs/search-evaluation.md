...
Novel Recall 同时提供关键词检索和剧情语义检索。关键词检索适合查找明确出现过的书名、作者、标签或简介词；剧情语义检索适合用自然语言描述想看的设定、人物关系或情节方向。通过固定 demo 数据和固定 query 评估，可以更稳定地观察语义搜索是否真的把“相近剧情”排在前面，而不是只依赖字面词命中。
...
建议先导入 `examples/demo-novels.json`，再在 options 页面为 demo 小说生成 AI 分析和向量，最后在 popup 中分别测试关键词检索与剧情检索。
...
| query | expectedTitle | keywordTop5Hit | semanticTop5Hit | semanticRank | notes |
| --- | --- | --- | --- | --- | --- |
|  |  | yes/no | yes/no |  |  |
